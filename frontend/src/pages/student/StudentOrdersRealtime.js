...
const statusConfig = {
  ...
  cancelled: { label: "Cancelled", color: "bg-red-300", badgeColor: "#FCA5A5", icon: XCircle },
};
...
export default function StudentOrdersRealtime() {
  ...
  const resolvedOrders = orders.filter((order) => ["completed", "cancelled"].includes(order.status));
    ...
    <div className="mobile-wrapper pb-8">
      ...
      {resolvedOrders.length > 0 && (
        <div className="px-5 pt-6">
          <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-gray-500 mb-3">History</h3>
          <div className="space-y-3">
            {resolvedOrders.map((order) => {
              const config = statusConfig[order.status] || statusConfig.completed;
              const StatusIcon = config.icon;
              const statusTextClass = order.status === "cancelled" ? "text-red-600" : "text-green-600";
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        ...
                        <div className={`flex items-center gap-1 ${statusTextClass}`}><StatusIcon className="w-4 h-4" strokeWidth={3} /><span className="text-xs font-bold">{config.label}</span></div>
                      </div>
